...
// cspell:disable AnalyserNode shockwaves SHOCKWAVES lerp

/*
    Developer resources & quick links

    - Local dev server (Vite): http://localhost:5173
    - Vite env docs: https://vitejs.dev/guide/env-and-mode.html
    - Web Audio API / AnalyserNode: https://developer.mozilla.org/en-US/docs/Web/API/AnalyserNode
    - Canvas API (2D rendering): https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API
    - AudioWorklet (advanced, low-latency processing): https://developer.mozilla.org/en-US/docs/Web/API/AudioWorklet

    If you'd prefer a UI 'Help' overlay with these links, I can add a tiny clickable icon
    to the visualizer that opens a short modal with these developer resources.
*/
...
const HIST_LEN = 256; // Fixed history length for radial graph
...
const hexToRgb = (hex: string) => {
  if (typeof hex !== 'string' || !/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.test(hex)) {
...
const AudioVisualizer: React.FC<AudioVisualizerProps> = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Smoother transition (0.1) instead of snappy (0.25) to prevent color flicker
      ...
      // Global Sensitivity Cap
      // Allows values to go high but not break the renderer
      maxVol = Math.min(1.5, maxVol); 
      ...
      // Smoother lerp factor for attack to prevent jitter
      // Was 0.3 / 0.08, now 0.15 / 0.05
      const lerpFactor = targetVolume > state.smoothedVolume ? 0.15 : 0.05; 
      state.smoothedVolume += (targetVolume - state.smoothedVolume) * lerpFactor;
      
      const v = state.smoothedVolume;
      const vBoost = v * 4.0; // Aggressive core deformation boost
      
      // Update Circular History Buffer (O(1))
      state.history[state.historyHead] = v;
      state.historyHead = (state.historyHead + 1) % HIST_LEN;

      if (v > 0.3 && v - state.lastVolume > 0.1) {
