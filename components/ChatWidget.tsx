                ...
                <div className="fixed bottom-8 inset-x-0 flex justify-center z-30">
                     <div className="flex items-center gap-2 px-3 py-2 bg-[#18181b] border border-white/10 rounded-full shadow-2xl">
                         ...
                         <ControlBtn 
                            ...
                            icon={isVideoActive ? <Video size={20} /> : <VideoOff size={20} className="text-gray-400" />}
                            label={isVideoActive ? "Camera On" : "Camera Off"}
                         />
                         ...
                         <ControlBtn 
                            ...
                            variant={'secondary'}
                            icon={<ArrowUpRight size={20} className="text-gray-400" />}
                            label="Share Screen"
                         />
          ...
          <div className="pointer-events-auto bg-[#FAFAFA] w-full max-w-6xl h-[90vh] sm:h-[85vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden relative border border-white/50">
            ...
            <div className="absolute bottom-0 inset-x-0 p-6 bg-white border-t border-gray-100 z-20">
                 <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-4 items-center">
                    ...
                    {/* Mic Selection - Only show if we have devices with labels (permission granted) */}
                    {audioDevices.length > 0 && audioDevices[0].label && (
                        <div className="relative w-full md:w-64 group">
                            ...
                            <select
                                ...
                                className="w-full pl-10 pr-8 py-3.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-100 focus:border-black appearance-none truncate"
                            >
                    ...
                    >
                        ...
                        <ArrowUpRight size={18} className={`transition-transform ${isEmailValid ? 'group-hover:translate-x-1 group-hover:-translate-y-1' : ''}`} />
                                        ...
                                        {/* Model chooser (runtime override — useful for testing model support) */}
                                        <div className="ml-3">
                                            <select
                                                value={forcedModel ?? 'auto'}
                                                onChange={(e) => setForcedModel(e.target.value === 'auto' ? null : e.target.value)}
                                                className="ml-3 text-sm rounded-lg border border-gray-200 bg-white/70 px-2 py-2 focus:outline-none"
                                                title="Runtime model selection (Auto / Multimodal / Audio-native)">
                                                <option value="gemini-live-2.5-flash-preview">Live Flash Preview (recommended)</option>
                                                <option value="gemini-2.5-flash-native-audio-preview-09-2025">Native Audio (high quality, slower)</option>
                                            </select>
                                        </div>
                 </div>
            </div>
